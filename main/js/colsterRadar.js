...
async function getLocRadarTimestamps() {
  //1 frame
  const response = await fetch(`https://api.weather.com/v3/TileServer/series/productSet?apiKey=e1f10a1e78da46f5b10a1e78da96f525&filter=waterTemperatureAtlanticPacific`);
  const data = await response.json();
  console.log('[waterTemp] loc seriesInfo keys:', JSON.stringify(data.seriesInfo.waterTemperatureAtlanticPacific.series[0]));
  const series = data.seriesInfo.waterTemperatureAtlanticPacific.series;
  const normalized = series.map(s => ({ ts: s.ts ?? s.validTime ?? s.sequenceId ?? Object.values(s)[0] }));
  locRadarTimestamps = normalized.sort((a, b) => a.ts - b.ts).slice(-1);
}
  ...
  for (let tstamp of locRadarTimestamps) {
    ...
    if (!radarEchoes.getSource(`radar_${timestamp}`)) {
      radarEchoes.addSource(`radar_${timestamp}`, {
        type: "raster",
        tiles: [`https://api.weather.com/v3/TileServer/tile/waterTemperatureAtlanticPacific?ts=${timestamp}&xyz={x}:{y}:{z}&apiKey=e1f10a1e78da46f5b10a1e78da96f525`],
        tileSize: 512,
        minzoom: 5,
        maxzoom: 12
      });
    ...
    if (!radarEchoes.getLayer(`radarlayer_${timestamp}`)) {
      radarEchoes.addLayer({
        ...
        paint: { "raster-opacity": 1, "raster-fade-duration": 0, "raster-brightness-min": 0, "raster-brightness-max": 1, "raster-saturation": 0, "raster-contrast": 0 }
      });
    }
  }
}
async function animateLocalRadar() {
  ...
  // Single frame — just show it immediately, no animation loop needed
  validLayers.forEach((layerId) => radarEchoes.setLayoutProperty(layerId, "visibility", "none"));
  radarEchoes.setLayoutProperty(validLayers[0], "visibility", "visible");
}
...
async function getSatRadarTimestamps() {
  ...
  const series = data.seriesInfo.waterTemperatureAtlanticPacific.series;
  const normalized = series.map(s => ({ ts: s.ts ?? s.validTime ?? s.sequenceId ?? Object.values(s)[0] }));
  satRadarTimestamps = normalized.sort((a, b) => a.ts - b.ts).slice(-1);
}
...
async function animateSatRadar() {
  ...
  satEchoes.setLayoutProperty(validLayers[0], "visibility", "visible");
}
...
async function getMiniRadarTimestamps() {
  ...
  const series = data.seriesInfo.waterTemperatureAtlanticPacific.series;
  const normalized = series.map(s => ({ ts: s.ts ?? s.validTime ?? s.sequenceId ?? Object.values(s)[0] }));
  miniRadarTimestamps = normalized.sort((a, b) => a.ts - b.ts).slice(-1);
}
